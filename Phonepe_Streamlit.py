...
required_packages = [
"pandas", "numpy", "json5", "os", "pymysql",
"seaborn", "matplotlib", "plotly", "nbformat", "streamlit"
]
...
st.set_page_config(
    ...
    page_icon=":moneybag:",  # Icon displayed in the browser tab (emoji, URL to an image, etc.)
...
def get_data(query, params=None):
    conn = pymysql.connect(host='127.0.0.1', user='root', passwd='Admin123',database="Phonepe")
    if params:
...
with col1:
    st.image("C:/Users/prasa/Jana/Phonepe/Logo/Phonepe_logo.png")
with col2:
    st.title("Phonepe Transaction Insights")
...
if r == "Home Page":
    ...
    **Database Used:** `Phonepe`
    ...
    if cat_select == "Transaction":
        ...
        #INDIA MAP
        df = pd.read_csv("C:/Users/prasa/Jana/Phonepe/csv/Agg_Transaction.csv")
        ...
        #INDIA MAP
        df = pd.read_csv("C:/Users/prasa/Jana/Phonepe/csv/Agg_Users.csv")
        df['State'] = df['State'].map(map_state)
        df['Year_Qtr'] = df['Year'].astype(str) + '_Q' + df['Quater'].astype(str)
        df1 = df[(df['Year_Qtr'] == yq_select)]
        state_list = df1['State'].unique()
